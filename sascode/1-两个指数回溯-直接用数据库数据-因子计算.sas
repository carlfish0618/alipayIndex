...
QUIT;
PROC SQL;
	CREATE TABLE fg_taobao_uni_med AS
...
/** 剔除不符合条件的股票 */
DATA fg_taobao_uni_med;
	SET fg_taobao_uni_med;
	IF stock_code IN ('600851', '002614', '600490', '300061') THEN delete;
	IF stock_code = '600849' AND end_date >= "28feb2010"d THEN delete;
RUN;

PROC SORT DATA = fg_taobao_uni_med;
	BY end_date stock_code;
RUN;

/*** Step1-3： 剔除不满足条件的股票 **/
%filter_stock_mdf(input_pool=fg_taobao_uni_med, output_pool=fg_taobao_uni_med);


/*** Step2: 计算淘宝得分 */ 
/**　输出：
(1) med_map(未来从外部导入)
(2) fg_taobao_st_score
**/

/** Step3-1: 淘宝行业匹配**/
%gen_stock_mapping(taobao_medicine_20150330.xlsx, med_map, TAOBAO_MEDICINE);
%gen_mapping_extend(adjust_table = adjust_busdate, input_table = med_map, start_date=15apr2013, 
...
PROC SQL;
	...
	WHERE stock_code IN 
		(SELECT stock_code FROM fg_taobao_uni_med) 
	ORDER BY end_date;
QUIT;
...
DATA tt;
	SET fgtest.fg_taobao_uni;
	IF stock_code = "600849";
RUN;
